...
def test_qidian_features_safe():
    ...
    try:
        ...
        for i, book in enumerate(books[:5], 1):
            ...
            print(f"     小说ID: {book.get('novel_id', '未知')}")
            if 'brief_intro' in book and book['brief_intro']:
                print(f"     简介: {book['brief_intro'][:50]}...")
            print()

        # 选择第一本书进行详细测试
        if books:
            first_book = books[0]
            print(f"\n3. 测试第一本书: 《{first_book['title']}》")
            print(f"   作者: {first_book.get('author', '未知')}")
            print(f"   分类: {first_book.get('category', '未知')}")
            print(f"   URL: {first_book['url']}")
            print(f"   平台ID: {first_book['novel_id']}")

            # 保存第一本书到数据库
            print("\n4. 保存第一本书到数据库...")
            try:
                result = db.save_daily_ranking(first_book)
                if result:
                    print(f"   成功保存: 《{first_book['title'][:20]}...》")

                    # 打印保存后的记录数
                    counts = db.get_table_counts()
                    print(f"   保存后表记录数: daily_rankings={counts.get('daily_rankings', 0)}")
                else:
                    print("   保存失败")
            except Exception as e:
                print(f"   保存失败: {e}")
            ...
            try:
                ...
                if detail:
                    ...
                    if 'introduction' in detail:
                        intro_preview = detail['introduction'][:100].replace('\n', ' ')
                        print(f"   简介预览: {intro_preview}...")
                    if 'tags' in detail and detail['tags']:
                        print(f"   标签: {', '.join(detail['tags'][:3])}")
                else:
                    print("   详情获取失败")
            except Exception as e:
                print(f"   详情补充测试失败: {e}")
                    ...
                    if chapters:
                        ...
                        for i, chapter in enumerate(chapters, 1):
                            ...
                            if chapter.get('chapter_content'):
                                content_preview = chapter['chapter_content'][:80].replace('\n', ' ')
                        ...
                        if hasattr(spider, 'db_handler') and spider.db_handler:
                            ...
                            novel_data = {
                                ...
                                'author': detail.get('author',
                                                     first_book.get('author', '未知')) if detail else first_book.get(
                                    'author', '未知'),
                                'platform': 'qidian',
                                'novel_url': first_book['url'],
                                'category': detail.get('category',
                                                       first_book.get('category', '')) if detail else first_book.get(
                                    'category', ''),
                            ...
                            if novel_saved:
                                ...
                                # 打印保存后的记录数
                                counts = db.get_table_counts()
                                print(f"   保存后表记录数: novel_archive={counts.get('novel_archive', 0)}, "
                                      f"novel_chapters={counts.get('novel_chapters', 0)}")

                                # 验证章节是否保存成功
                                db_chapters = db.get_novel_chapters(first_book['novel_id'], 3)
                                print(f"   从数据库读取前3章: {len(db_chapters)} 章")
            ...
            try:
                if hasattr(spider, 'enrich_books_with_details'):
                    print("   使用enrich_books_with_details补充详细信息...")
                    ...
                    if enriched_books:
                        print(f"   成功补充 {len(enriched_books)} 本书的详细信息")
            ...
            try:
                ...
                with db._db_lock:
                    ...
                    # 查询已保存的小说基本信息
                    cursor.execute('''
                        SELECT novel_id, title, author, category, chapters_count
                        FROM novel_archive 
